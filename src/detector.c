...
*/
char* get_filename(char *pic_abs_path){
    char *abs_path = pic_abs_path;
    char *filename = strchr(abs_path, '/');
    if(filename) {
        return filename + 1;
    }
    else {
...
//以下就是我重写的主要内容，将循环改成了下面这个样子
    if (filename) {
        ...
        char *picture = pic;
        int countpic=1;
        FILE *picturepath = fopen(input, "r");
//这里是save_labels的文件，用于保存文本的预测信息，在这个路径下的这个txt文件，这下面的路径如果不匹配的话自行修改下方这个fopen路径
        FILE* fw = fopen("/home/pgh/work/darknet/result.txt", "wb");
        while(!feof(picturepath)){
            fgets(pic,sizeof(pic),picturepath);
            int a,b,c,d=0;
            c=strlen(pic);
            for(a=0;a<c-d;a++){
                if(pic[a]=='\n'){
            ...
            image im = load_image(pic,0,0,net.c);
            image sized;
            int letterbox = 0;
            if(letter_box) sized=letterbox_image(im, net.w, net.h);
            else sized = resize_image(im, net.w, net.h);
            ...
            double time = get_time_point();
            network_predict(net, X);
            printf("%s: Predicted in %lf milli-seconds.\n", pic, ((double)get_time_point() - time) / 1000);
            int nboxes = 0;
            detection *dets = get_network_boxes(&net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes, letter_box);
            if (nms) {
                if (l.nms_kind == DEFAULT_NMS) do_nms_sort(dets, nboxes, l.classes, nms);
            ...
            draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output);
            char picture_save_name[1024];
//下面是保存输出预测结果图像的位置，也可以根据自己需要进行修改，要确保自己电脑上存在这个路径
	    //sprintf(picture_save_name,"/home/pgh/work/image/output/%06d.jpg",countpic);
        //countpic=countpic+1;
        char *pic_name = get_filename(pic);
        if(pic_name) {
            sprintf(picture_save_name,"/home/pgh/work/image/output/%s.jpg",pic_name);
        }
        else {
            printf("Error: pic_name is null.\n");
        }
        if(nboxes > 0){
            save_image(im,picture_save_name);
            printf("save %s successfully!\nsave %s as %s\n",pic,pic,picture_save_name);
        }
        //else {
        //    save_image(im,picture_save_name);
        //}
//这下面是save_labels的函数，我在这里是写死的三个类型名称，有兴趣可以把这里的注释解掉，自己修改名称，save_labels是用来保存文本信息的，和图像没有关系，如果需要保存文本，目前这个版本没法写入置信度，其他必要信息都有
//            if (save_labels)
//            {
//                char labelpath[4096];
//                replace_image_to_label(picture, labelpath);
//                char buff0[1024];
//                sprintf(buff0,"%s \n",picture_save_name);
//                fwrite(buff0,sizeof(char),strlen(buff0),fw);
//                int i;
//                if(nboxes>0)quexian_count=quexian_count+1;
//                for (i = 0; i < nboxes; ++i) {
//                    char buff[1024];
//                    int class_id = -1;
//                    float prob = 0;
...
//                    if (class_id >= 0) {
//这里是我把class_id转换成类型名的步骤，根据有多少类，然后1、2、3这种和你的*.names里面的类名一一对应就可以了
//                        if (class_id==1)sprintf(class_name,"%s","mosaic");
//                        if (class_id==2)sprintf(class_name,"%s","heng_tiao");
//                        if (class_id==3)sprintf(class_name,"%s","hui_ping");
//                        printf("classname = %s\n",class_name);
//如果不需要输出class_id，就将下面这句改为：sprintf(buff, "%s: %2.4f %2.4f %2.4f %2.4f\n", class_name, dets[i].bbox.x, dets[i].bbox.y, dets[i].bbox.w, dets[i].bbox.h);
//                        sprintf(buff, "%s: %d %2.4f %2.4f %2.4f %2.4f\n", class_name, class_id, dets[i].bbox.x, dets[i].bbox.y, dets[i].bbox.w, dets[i].bbox.h);
//                        fwrite(buff, sizeof(char), strlen(buff), fw);
//                    }
//                }
//            }
...
//        char buff1[1024];
//        sprintf(buff1,"All_Quexian_Picture_Count: %d \n",quexian_count);
//        printf("%d \n",quexian_count);
//        fwrite(buff1, sizeof(char),strlen(buff),fw);
        fclose(fw);
        fclose(picturepath);
    if (json_file) {
        char *tmp = "\n]";
